...
#define declareFunctionHook(returnType, functionName, ...) \
        returnType (* real_gecko_ ## functionName)(__VA_ARGS__) __attribute__((section(".data"))); \
...
#define makeFunctionHook(functionName, library, functionType) { (unsigned int) my_ ## functionName, (unsigned int) &real_gecko_ ## functionName, library, # functionName,0,0,functionType,0}
...
bool isDynamicFunction_gecko(unsigned int physicalAddress);
